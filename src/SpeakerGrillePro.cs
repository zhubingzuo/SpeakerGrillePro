...
namespace SpeakerGrillePro
{
    ...
    public class SwAddin : ISwAddin
    {
        ...
        private const int CmdGroupId = 48522;
        ...
        public bool ConnectToSW(object ThisSW, int Cookie)
        {
            ...
            try
            {
                ...
                {
                    // v27: startup is intentionally silent. Keep the add-in loaded even if
                    // the toolbar/menu API fails, and write diagnostics only to the runtime log.
                    RuntimeLog.Write("CommandManager creation FAILED: " + uiEx);
                }

                RuntimeLog.Write("CONNECT_OK");
                RuntimeLog.Write("STARTUP_SILENT v27: no load-success dialog is shown.");
                return true;
            }
            catch (Exception ex)
            {
                // v27: do not display modal windows during SOLIDWORKS startup.
        ...
        private void AddCommandManager()
        {
            ...
            // v27: unified multi-style fixed-size add-in. Remove legacy standalone honeycomb command groups
            // so the user sees one unified generator instead of two separate toolbars.
            // v27.2 icon refresh: remove the previous command group ID once so
            // SOLIDWORKS does not keep using its cached toolbar image.
            try { _cmdMgr.RemoveCommandGroup(48521); } catch { }
            ...
            // v27.2: use a dedicated loudspeaker icon at all SOLIDWORKS high-DPI sizes.
            // IconList is the toolbar strip; because this group has one command, each file
            // contains exactly one icon. MainIconList uses the same single-icon files.
            string[] iconPaths = GetToolbarIconPaths();
            if (iconPaths != null && iconPaths.Length > 0)
            {
                group.IconList = iconPaths;
                group.MainIconList = iconPaths;
                RuntimeLog.Write("Toolbar speaker icons loaded from: " + Path.GetDirectoryName(iconPaths[0]));
            }
            else
            {
                RuntimeLog.Write("Toolbar speaker icons were not found; SOLIDWORKS default icon will be used.");
            }
        ...
        private string[] GetToolbarIconPaths()
        {
            try
            {
                string assemblyDir = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
                string iconDir = Path.Combine(assemblyDir, "Icons");
                int[] sizes = new int[] { 20, 32, 40, 64, 96, 128 };
                string[] paths = new string[sizes.Length];
                for (int i = 0; i < sizes.Length; i++)
                {
                    paths[i] = Path.Combine(iconDir, "speaker_" + sizes[i].ToString(CultureInfo.InvariantCulture) + ".png");
                    if (!File.Exists(paths[i]))
                    {
                        RuntimeLog.Write("Missing toolbar icon: " + paths[i]);
                        return null;
                    }
                }
                return paths;
            ...
            {
                RuntimeLog.Write("GetToolbarIconPaths failed: " + ex.Message);
                return null;
            }
        }
        ...
        private void CreateGrille(ModelDoc2 model, Face2 face, double[] modelCenter, GrilleSettings s)
        {
            ...
            // v27 fixed-size sizing with selectable rectangular/circular grille region.
            ...
            // ShapeMode 7 is a dedicated premium style: circular footprint + staggered round holes
            // + continuous diameter gradient + no skipped instances. Lock these invariants in the
            // geometry layer too, so programmatic callers cannot accidentally break the style.
            if (s.ShapeMode == 7)
            {
                s.RegionMode = 1;
                s.EdgeSkipPercent = 0.0;
                RuntimeLog.Write("PREMIUM_CIRCULAR_GRADIENT locked: circle region + continuous gradient + no skip");
            }

            double W = Mm(s.WidthMm), H = Mm(s.HeightMm), corner = Mm(s.CornerRadiusMm);
            double circleD = Mm(s.CircleDiameterMm);
            if (s.RegionMode == 1)
            {
                W = circleD;
                H = circleD;
            ...
            RuntimeLog.Write("REGION_MODE " + (s.RegionMode == 1 ? "CIRCLE" : "ROUNDED_RECT") +
                (s.RegionMode == 1 ? (" diameter_mm=" + s.CircleDiameterMm.ToString("F3", CultureInfo.InvariantCulture)) :
                (" width_mm=" + s.WidthMm.ToString("F3", CultureInfo.InvariantCulture) + ", height_mm=" + s.HeightMm.ToString("F3", CultureInfo.InvariantCulture))));
            ...
            double soundWaveWebEff = Mm(Clamp(s.SoundWaveWebMm, 0.80, 5.0));
            ...
            Func<double, double> sizeForRho = delegate(double rho)
            {
                if (s.ShapeMode == 6)
                {
                    // v27.3 aesthetic sound-wave profile: continuous, zero-slope transitions
                    // instead of three abrupt diameter bands. D1 -> D2 -> D3 follows smoothstep.
                    double t = Math.Max(0.0, Math.Min(1.0, rho));
                    Func<double, double, double, double> smoothLerp = delegate(double a, double b, double u)
                    {
                    ...
                    if (t <= s.CenterZone)
                        return smoothLerp(centerSize, middleSize, t / Math.Max(1e-9, s.CenterZone));
                    if (t <= s.MiddleZone)
                        return smoothLerp(middleSize, outerSize, (t - s.CenterZone) / Math.Max(1e-9, s.MiddleZone - s.CenterZone));
                    return outerSize;
                }
                if (s.ShapeMode == 7)
                {
                    // Continuous cosine easing: maximum diameter at the center, minimum at the rim,
                    // with zero slope at both ends. This avoids visible 3-step diameter bands.
                    double t = Math.Max(0.0, Math.Min(1.0, rho));
                    double weight = 0.5 * (1.0 + Math.Cos(Math.PI * t));
                    return outerSize + (centerSize - outerSize) * weight;
                }
            ...
            if (s.ShapeMode == 7)
            {
                // Premium circular grille: triangular/hex-staggered lattice, continuously varying
                // round-hole diameter and no skipped holes. The boundary test uses EACH hole's own
                // radius so the small outer holes can approach the circular rim without the overly
                // conservative clearance caused by using the largest center-hole radius everywhere.
                double pitch = pitchEff;
                double dy = pitch * Math.Sqrt(3.0) / 2.0;
                double regionR = W * 0.5;
                maxR = maxFeature * 0.5;
                skipPitch = pitch;
                ...
                for (int row = -rowMax; row <= rowMax; row++)
                {
                    ...
                    for (int col = -colMax; col <= colMax; col++)
                    {
                        ...
                        double rho = radial / Math.Max(1e-9, regionR);
                        if (rho > 1.0) continue;
                        double dia = sizeForRho(rho);
                        double hr = dia * 0.5;
                        // Entire circular hole must stay inside the user-defined circular grille region.
                        if (radial + hr > regionR + 1e-9) continue;
                        holes.Add(new Hole(cx + x, cy + y, hr));
                    }
                }
                RuntimeLog.Write(string.Format(CultureInfo.InvariantCulture,
                    "PATTERN_MODE PREMIUM_CIRCULAR_CONTINUOUS pitch_mm={0:F3}, diameter_center_mm={1:F3}, diameter_edge_mm={2:F3}, holes={3}",
                    pitch * 1000.0, centerSize * 1000.0, outerSize * 1000.0, holes.Count));
            }
            else if (s.ShapeMode == 0 || s.ShapeMode == 2)
            {
                ...
                for (int row = -rowMax; row <= rowMax; row++)
                {
                    ...
                    {
                        double x = col * pitch + xShift;
                        if (!InsideConfiguredRegion(x, y, W, H, corner, maxR, s.RegionMode)) continue;
                        double rho = RegionRho(x, y, W, H, s.RegionMode);
                        double dia = sizeForRho(rho);
                        bool inSkipShell = IsInOutermostSkipShellRegion(x, y, W, H, corner, maxR, pitch, s.SkipStartZone, s.RegionMode);
                        bool protectedRoundedCorner = s.RegionMode == 0 && IsProtectedRoundedCorner(x, y, W, H, corner, maxR, pitch);
            ...
            else if (s.ShapeMode == 1)
            {
                ...
                for (int col = -colMax; col <= colMax; col++)
                {
                    ...
                    for (int row = -rowMax; row <= rowMax; row++)
                    {
                        ...
                        bool inSkipShell = IsInOutermostSkipShellRegion(x, y, W, H, corner, maxR, neighbor, s.SkipStartZone, s.RegionMode);
                        bool protectedRoundedCorner = s.RegionMode == 0 && IsProtectedRoundedCorner(x, y, W, H, corner, maxR, neighbor);
            ...
            {
                // v27.3 BEAUTIFIED RADIAL SOUND WAVE
                // Design goals:
                // 1) clean fourfold symmetry on every complete ring;
                // 2) smoothly varying hole diameter (no visible 3-band jumps);
                // 3) hard minimum plastic web both between adjacent rings and between holes on a ring;
                // 4) ring populations progress in stable multiples of four (8, 12, 16, 20...) rather
                //    than arbitrary 6/13/19 counts that look visually noisy.
                double pitch = pitchEff;
                double web = soundWaveWebEff;
                maxR = maxFeature * 0.5;
                skipPitch = pitch;
                double maxRing = s.RegionMode == 1 ? W * 0.5 : Math.Sqrt((W * 0.5) * (W * 0.5) + (H * 0.5) * (H * 0.5));

                double centerHr = sizeForRho(0.0) * 0.5;
                holes.Add(new Hole(cx, cy, centerHr));

                double prevRingR = 0.0;
                double prevHr = centerHr;
                int ring = 1;
                while (ring < 256)
                {
                    // First estimate from the nominal pitch, then enlarge the radial step whenever
                    // required to preserve the requested solid web between two neighboring rings.
                    double rrGuess = prevRingR + Math.Max(pitch, prevHr + outerSize * 0.5 + web);
                    double rhoGuess = RegionRho(rrGuess, 0.0, W, H, s.RegionMode);
                    double dia = sizeForRho(rhoGuess);
                    double hr = dia * 0.5;
                    double rr = prevRingR + Math.Max(pitch, prevHr + hr + web);

                    // Re-evaluate diameter once using the final radius; this keeps the smooth gradient
                    // consistent even when the web constraint pushes a ring slightly outward.
                    double rho = RegionRho(rr, 0.0, W, H, s.RegionMode);
                    dia = sizeForRho(rho);
                    hr = dia * 0.5;
                    rr = prevRingR + Math.Max(pitch, prevHr + hr + web);
                    rho = RegionRho(rr, 0.0, W, H, s.RegionMode);
                    dia = sizeForRho(rho);
                    hr = dia * 0.5;

                    if (rr - hr > maxRing + 1e-9) break;

                    double minChord = 2.0 * hr + web;
                    double desiredStep = Math.Max(pitch, minChord);
                    int targetN = Math.Max(8, (int)Math.Round(2.0 * Math.PI * rr / desiredStep));
                    int n = Math.Max(8, 4 * (int)Math.Floor(targetN / 4.0 + 0.5));

                    // Exact chord check. If the rounded population is too dense, reduce it by four.
                    while (n > 8 && 2.0 * rr * Math.Sin(Math.PI / n) < minChord - 1e-9) n -= 4;

                    // For the innermost ring, eight holes look much better than a four-hole cross.
                    // If eight do not yet fit safely, move that ring outward just enough.
                    double minRadiusForEight = minChord / (2.0 * Math.Sin(Math.PI / 8.0));
                    if (n == 8 && rr < minRadiusForEight)
                    {
                        rr = minRadiusForEight;
                        rho = RegionRho(rr, 0.0, W, H, s.RegionMode);
                        dia = sizeForRho(rho);
                        hr = dia * 0.5;
                        minChord = 2.0 * hr + web;
                        minRadiusForEight = minChord / (2.0 * Math.Sin(Math.PI / 8.0));
                        rr = Math.Max(rr, minRadiusForEight);
                    }

                    // Actual-hole boundary check uses each hole's own radius instead of the largest
                    // center-hole radius, allowing a clean outer contour without weakening the rim.
                    ...
                    for (int i = 0; i < n; i++)
                    {
                        ...
                        if (!InsideConfiguredRegion(x, y, W, H, corner, hr, s.RegionMode)) continue;
                        holes.Add(new Hole(cx + x, cy + y, hr));
                        addedThisRing++;
                    }

                    // Stop after a completely clipped ring; later rings cannot recover on a circular region.
                    if (addedThisRing == 0 && s.RegionMode == 1) break;

                    prevRingR = rr;
                    prevHr = hr;
                    ring++;
                    if (rr > maxRing + Math.Max(pitch, dia + web)) break;
                }
                RuntimeLog.Write(string.Format(CultureInfo.InvariantCulture,
                    "PATTERN_MODE RADIAL_WAVE_BEAUTY_V273 pitch_mm={0:F3}, min_web_mm={1:F3}, rings={2}, holes={3}",
                    pitch * 1000.0, web * 1000.0, ring - 1, holes.Count));
            }

            RuntimeLog.Write("EDGE_SKIP v27 skipped=" + skippedEdgeCount +
            ...
            foreach (Hole h in holes)
            {
                ...
                if (s.ShapeMode == 6)
                {
                    // v27.3: keep a sound-wave hole only when its complete
                    // mirror orbit is supported by the real trimmed face. This prevents tiny
                    // face-boundary/tolerance differences from leaving an unmatched hole on
                    // only one side. The final pattern therefore stays bilaterally symmetric
                    // about both axes through the user's selected center point.
                    double dx = h.X - cx;
                    double dy = h.Y - cy;
                    ok = IsCircleFullySupportedByFace(face, sketchToModel, mu, cx + dx, cy + dy, h.R)
                        && IsCircleFullySupportedByFace(face, sketchToModel, mu, cx - dx, cy + dy, h.R)
                        && IsCircleFullySupportedByFace(face, sketchToModel, mu, cx + dx, cy - dy, h.R)
                        && IsCircleFullySupportedByFace(face, sketchToModel, mu, cx - dx, cy - dy, h.R);
                }
                else if (s.ShapeMode == 0 || s.ShapeMode == 2 || s.ShapeMode == 7)
            ...
            try
            {
                foreach (Hole h in holes)
                {
                    if (s.ShapeMode == 0 || s.ShapeMode == 2 || s.ShapeMode == 6 || s.ShapeMode == 7)
                    {
            ...
            RuntimeLog.Write(((s.ShapeMode == 0 || s.ShapeMode == 2 || s.ShapeMode == 6 || s.ShapeMode == 7) ? "CIRCLE_CREATE" : "POLYGON_CREATE") +
                " requested=" + holes.Count + ", created=" + createdProfileCount +
                ((s.ShapeMode == 0 || s.ShapeMode == 2 || s.ShapeMode == 6 || s.ShapeMode == 7) ? "" : ", segments=" + createdSegmentCount));
            if (createdProfileCount != holes.Count)
                RuntimeLog.Write("PROFILE_CREATE WARNING: one or more profiles were not fully created.");

            // v27 PRIMARY CUT PATH:
        ...
        private static int CreateRegularHexagon(SketchManager skMgr, double cx, double cy, double r)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                // Flat-top regular hexagon. Combined with the v27 axial lattice, neighboring
        ...
        private static bool InsideConfiguredRegion(double x, double y, double width, double height, double cornerRadius, double featureRadius, int regionMode)
        {
            if (regionMode == 1)
            {
                double usableR = width * 0.5 - featureRadius;
                if (usableR <= 0) return false;
                return x * x + y * y <= usableR * usableR;
            }
            return InsideRoundedRect(x, y, width * 0.5 - featureRadius, height * 0.5 - featureRadius, Math.Max(0.0, cornerRadius - featureRadius));
        }

        private static double RegionRho(double x, double y, double width, double height, int regionMode)
        {
            if (regionMode == 1)
                return Math.Sqrt(x * x + y * y) / Math.Max(1e-9, width * 0.5);
            double nx = x / Math.Max(1e-9, width * 0.5);
            double ny = y / Math.Max(1e-9, height * 0.5);
            return Math.Sqrt(nx * nx + ny * ny);
        }

        private static bool IsInOutermostSkipShellRegion(
            double x, double y, double width, double height, double cornerRadius,
            double maxHoleRadius, double pitch, double skipStartZone, int regionMode)
        {
            if (regionMode == 1)
            {
                double outer = width * 0.5 - maxHoleRadius;
                if (outer <= 0) return false;
                double radial = Math.Sqrt(x * x + y * y);
                double shell = Math.Max(pitch * 1.15, maxHoleRadius * 2.0);
                double inner = Math.Max(0.0, outer - shell);
                double gated = Math.Max(inner, outer * Math.Max(0.0, Math.Min(1.0, skipStartZone)));
                return radial >= gated && radial <= outer + 1e-9;
            }
            return IsInOutermostSkipShell(x, y, width, height, cornerRadius, maxHoleRadius, pitch, skipStartZone);
        }
    ...
    public sealed class GrilleSettings
    {
        ...
        // 7 circular-region staggered round holes with continuous diameter gradient and no skipping.
        public int ShapeMode = 0;
        // RegionMode: 0 = rounded rectangle, 1 = circle.
        public int RegionMode = 0;
        public double CircleDiameterMm = 60;
        public double HoneycombWebMm = 0.55;
        public double SoundWaveWebMm = 1.20;
    ...
    public sealed class GrilleDialog : Form
    {
        ...
        private ComboBox _regionCombo;
        private int _nextFieldRow = 3;
        public GrilleSettings Settings { get; private set; }

        public GrilleDialog()
        {
            Text = "喇叭孔生成器 Pro v27.3 - 同心声波美学优化 + 安全肉厚";
            Width = 560;
            Height = 790;
            ...
            var panel = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 3, RowCount = 21, Padding = new Padding(14), AutoSize = false };
            ...
            AddHeader(panel, "v27.3：同心声波美学优化 + 最小肉厚保护 + 8 种孔型");
            AddShapeSelector(panel);
            AddRegionSelector(panel);
            AddField(panel, "区域宽度", "Width", 80, "mm");
            AddField(panel, "区域高度", "Height", 44, "mm");
            AddField(panel, "外轮廓圆角（矩形区域）", "Corner", 12, "mm");
            AddField(panel, "圆形区域直径", "CircleDiameter", 60, "mm");
            AddField(panel, "孔中心距 / 基准节距", "Pitch", 3.0, "mm");
            AddField(panel, "蜂窝筋宽（蜂窝模式）", "HoneyWeb", 0.55, "mm");
            AddField(panel, "同心声波最小孔间肉厚", "WaveWeb", 1.20, "mm");
            ...
            var info = new Label
            {
                Text = "v27 保留‘圆形渐变 - 六角错列连续孔径’专用样式：自动锁定圆形区域，中心孔尺寸作为最大孔径、外围孔尺寸作为最小孔径，孔径按半径连续平滑变化，并强制不跳孔。该样式中间孔尺寸、两级区域比例和跳孔参数自动停用。\n\n“圆孔 - 同心声波”已升级为连续孔径过渡 + 四重对称环阵列，并把“最小孔间肉厚”作为硬约束；默认 1.20 mm，建议塑料件通常保持 1.0~1.5 mm 或更大。\n\n其他 7 种样式继续支持圆角矩形 / 圆形区域。特征尺寸定义：圆孔=直径；蜂窝/方孔/菱形=对边尺寸；三角孔=外接圆直径。",
        ...
        private void AddShapeSelector(TableLayoutPanel p)
        {
            ...
            _shapeCombo.Items.Add("圆形渐变 - 六角错列连续孔径（推荐）");
            _shapeCombo.SelectedIndex = 0;
            var u = new Label { Text = "8 种", Anchor = AnchorStyles.Left, AutoSize = true, ForeColor = System.Drawing.Color.DimGray };
        ...
        private void AddRegionSelector(TableLayoutPanel p)
        {
            int r = 2;
            var l = new Label { Text = "生成区域形状", Anchor = AnchorStyles.Left, AutoSize = true };
            _regionCombo = new ComboBox { Dock = DockStyle.Fill, DropDownStyle = ComboBoxStyle.DropDownList };
            _regionCombo.Items.Add("圆角矩形区域（宽 × 高）");
            _regionCombo.Items.Add("圆形区域（按直径）");
            _regionCombo.SelectedIndex = 0;
            var u = new Label { Text = "区域", Anchor = AnchorStyles.Left, AutoSize = true, ForeColor = System.Drawing.Color.DimGray };
            p.Controls.Add(l, 0, r); p.Controls.Add(_regionCombo, 1, r); p.Controls.Add(u, 2, r);
            _regionCombo.SelectedIndexChanged += delegate { ApplyRegionMode(); };
        }

        private void ApplyRegionMode()
        {
            if (_regionCombo == null || !_boxes.ContainsKey("Width") || !_boxes.ContainsKey("CircleDiameter")) return;
            bool circle = _regionCombo.SelectedIndex == 1;
            _boxes["Width"].Enabled = !circle;
            _boxes["Height"].Enabled = !circle;
            _boxes["Corner"].Enabled = !circle;
            _boxes["CircleDiameter"].Enabled = circle;
        }

        private void ApplyShapeModeUi()
        {
            if (_shapeCombo == null || _regionCombo == null || !_boxes.ContainsKey("D2")) return;
            bool continuousCircular = _shapeCombo.SelectedIndex == 7;
            bool soundWave = _shapeCombo.SelectedIndex == 6;
            if (_boxes.ContainsKey("WaveWeb")) _boxes["WaveWeb"].Enabled = soundWave;
            if (continuousCircular)
            {
                // This dedicated style is intentionally locked to a circular footprint and no skipping.
                _regionCombo.SelectedIndex = 1;
                _regionCombo.Enabled = false;
                _boxes["D2"].Enabled = false;
                _boxes["Z1"].Enabled = false;
                _boxes["Z2"].Enabled = false;
                _boxes["SkipStart"].Enabled = false;
                _boxes["SkipPct"].Enabled = false;
                _boxes["SkipPct"].Text = "0";
            }
            else
            {
                _regionCombo.Enabled = true;
                _boxes["D2"].Enabled = true;
                _boxes["Z1"].Enabled = true;
                _boxes["Z2"].Enabled = true;
                _boxes["SkipStart"].Enabled = true;
                _boxes["SkipPct"].Enabled = true;
            }
            ApplyRegionMode();
        }
        ...
        private void ApplyPreset(int mode)
        {
            ...
            else if (mode == 6) { Set("D1",2.0); Set("D2",1.5); Set("D3",1.0); Set("Pitch",3.2); Set("WaveWeb",1.20); Set("Corner",16); }
            else
            {
                // Dedicated premium circular grille: continuous cosine-eased diameter gradient,
                // staggered hex lattice and absolutely no skipped holes.
                Set("CircleDiameter",60); Set("D1",2.2); Set("D2",1.6); Set("D3",1.0);
                Set("Pitch",3.0); Set("SkipPct",0); Set("SkipStart",0.94);
                if (_regionCombo != null) _regionCombo.SelectedIndex = 1;
            }
            ApplyShapeModeUi();
        }
        ...
        private void ValidateAndClose()
        {
            try
            {
                var s = new GrilleSettings
                {
                    ...
                    RegionMode = _regionCombo == null ? 0 : _regionCombo.SelectedIndex,
                    CircleDiameterMm = V("CircleDiameter"),
                    HoneycombWebMm = V("HoneyWeb"),
                    SoundWaveWebMm = V("WaveWeb"),
                ...
                if (s.RegionMode == 0 && (s.WidthMm <= 0 || s.HeightMm <= 0)) throw new Exception("矩形区域的宽度和高度必须大于 0。\n");
                if (s.RegionMode == 1 && s.CircleDiameterMm <= 0) throw new Exception("圆形区域直径必须大于 0。\n");
                if (s.CenterDiaMm <= 0 || s.MiddleDiaMm <= 0 || s.OuterDiaMm <= 0) throw new Exception("孔尺寸必须大于 0。\n");
                if (s.ShapeMode == 7)
                {
                    if (s.CenterDiaMm < s.OuterDiaMm) throw new Exception("连续渐变模式应保持：中心孔尺寸 ≥ 外围孔尺寸。\n");
                }
                else if (!(s.CenterDiaMm >= s.MiddleDiaMm && s.MiddleDiaMm >= s.OuterDiaMm))
                    throw new Exception("建议保持：中心孔尺寸 ≥ 中间孔尺寸 ≥ 外围孔尺寸。\n");
                ...
                if (s.ShapeMode == 6)
                {
                    if (s.SoundWaveWebMm < 0.8) throw new Exception("同心声波最小孔间肉厚不能小于 0.8 mm。塑料件建议使用 1.0~1.5 mm，默认 1.2 mm。\n");
                    if (s.SoundWaveWebMm > 5.0) throw new Exception("同心声波最小孔间肉厚设置过大，请使用 0.8~5.0 mm。\n");
                }
                if (s.ShapeMode != 7)
                {
